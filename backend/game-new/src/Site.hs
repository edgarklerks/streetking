...
{-- till here --}
wrapErrors x = runDb (forkSqlTransaction $ forkSqlTransaction $  Task.run Task.Cron 0) >>  CIO.catch (CIO.catch x (\(UserErrorE s) -> writeError s)) (\(e :: SomeException) -> writeError (show e))
