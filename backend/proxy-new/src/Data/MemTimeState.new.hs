...
unlockFile ms = atomically $ putDVar (_filelock ms) ()

lockFile :: MemState -> IO ()
lockFile ms = atomically $ takeDVar (_filelock ms)

-- | Blocking lock action 
withLock :: MemState -> IO a -> IO a 
withLock ms a = do s <- atomically $ takeDVar (_filelock ms)
                   x <- a
                   atomically $ putDVar (_filelock ms) s
                   return x 


-- | Non blocking lock action 
ifUnlock :: MemState -> IO a -> IO ()
ifUnlock ms a = do s <- atomically $ tryTakeDVar (_filelock ms) 
                   case s of 
                        Nothing -> return ()
                        Just l -> a >> atomically (putDVar (_filelock ms) l)

mkTLens :: Functor f => (a -> TVar b) -> RefFamily f a (STM a) (STM  b) b  
mkTLens f = mkLens out inp 
    where out a = readDVar (f a) 
          inp a b = writeDVar (f a) b >> return a 
...
runQuery s x = do 
            un <- newEmptyDVar 
            atomically $ writeDVar s (x,un)
            atomically $ takeDVar un 
...
queryManager fp ms qc = forkIO (sweeper ms) >> (forever $ do 
                            ...
                            case q of 
                                ...
                                Query a -> atomically $ do 
                                                ...
                                                case v of 
                                                    Just a -> putDVar u (Value a)
                                                    Nothing -> putDVar u NotFound 

                                DumpState -> storeSnapShot fp ms >> (atomically $ putDVar u Empty)) 
