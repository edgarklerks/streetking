...
foldTDTree :: (Relation -> b -> b) -> b -> Relation -> b
foldTDTree f z r = undefined  

foldBUTree :: (b -> Relation -> b) -> b -> Relation -> b 
foldBUTree f z (Relation s q) =  f z (Relation s q) 
foldBUTree f z t@(Projection s r) = let z' = foldBUTree f z r
                                    in f z' t
foldBUTree f z t@(Selection c r) = let z' = foldBUTree f z r
                                   in f z' t

foldBUTree f z t@(Union rl rr) = let z' = foldBUTree f z rl
                                     z'' = foldBUTree f z' rr 
                                 in f z'' t 
foldBUTree f z t@(Intersection rl rr) = let z' = foldBUTree f z rl 
                                            z'' = foldBUTree f z' rr 
                                        in f z'' t 
foldBUTree f z t@(SetDifference rl rr) = let z' = foldBUTree f z rl 
                                             z'' = foldBUTree f z rr 
                                         in f z'' t 
foldBUTree f z t@(CrossProduct rl rr) = let z' = foldBUTree f z rl 
                                            z'' = foldBUTree f z' rr
                                         in f z'' t 
foldBUTree f z t@(Rename k1 k2 r) = let z' = foldBUTree f z r
                                    in f z' t 
foldBUTree f z t@(Join c rl rr) = let z' = foldBUTree f z rl
                                      z'' = foldBUTree f z' rr
                                  in f z'' t 
foldBUTree f z t@(Take i r) = let z' = foldBUTree f z r
                              in f z' t 
foldBUTree f z t@(Drop i r) = let z' = foldBUTree f z r
                              in f z' t


getRenames = foldBUTree step [] 
    where step z (Rename p x _) = (p,x) : z
          step z r  = z
...
isType (Relation _ _) = TRelation
isType (Projection _ _) = TProjection 
isType (Selection _ _) = TSelection 
isType (Union _ _) = TUnion 
isType (Intersection _ _) = TIntersection 
isType (Join _ _ _) = TJoin
isType (Rename _ _ _) = TRename

testQuery = let phase1 = walkTDRelation step (exec testJoin) in phase1 
        where step TRename = renameTransform1 
              step TJoin = renameTransform1 
              step _ = id  


renameTransform1 (Rename k v r) = Rename v k r 
renameTransform1 (Join c r r') = Join Always r r'

walkTDRelation:: (TKey -> Relation -> Relation) -> Relation -> Relation 
walkTDRelation f r =  case f (isType r) r of
                              (Relation s q) -> Relation s q 
                              (Projection s r) -> Projection s (walkTDRelation f r)
                              (Selection c r) -> Selection c (walkTDRelation f r)
                              (Union rl rr) -> Union (walkTDRelation f rl) (walkTDRelation f rr)
                              (Intersection rl rr) -> Intersection (walkTDRelation f rl) (walkTDRelation f rr)
                              (Join c rl rr) -> Join c (walkTDRelation f rl) (walkTDRelation f rr)
                              (Rename k v r) -> Rename k v (walkTDRelation f r) 
...
-- Join cp r1 r2 :: IR -> 
--
-- Annotate (Join cp r1 r2) (Join l/r/o/i r1 (Join r2a r2b))
--
-- optimize_(mysql|postgresql) :: IR -> IR 
--
-- IR -> mysql ...
-- IR -> postgresql code 
-- IR -> lists 
