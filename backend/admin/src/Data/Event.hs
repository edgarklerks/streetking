...
data Event where 
    ...
    ChallengeRace :: Integer -> Integer -> Event 
    -- Level 
    Level :: Integer -> Event      
        deriving (Show, Eq)
...
instance ToInRule Event where 
    ...
    toInRule (ChallengeRace p t) = InArray [InString "R", InInteger p, InInteger t]
...
instance FromInRule Event where 
    ...
    fromInRule (InArray [InString "R", InInteger p, InInteger t]) = ChallengeRace p t 
...
instance AS.FromJSON Event where 
        parseJSON (AS.Array x) = case x V.! 0 of 
                                    ...
                                    (AS.String "R") -> return $ ChallengeRace (toInt $ x V.! 1) (toInt $ x V.! 2)
...
instance Evaluate Event Symbol where 
    ...
    match (PracticeRace track_id) (PracticeI x) = cmp x track_id 
    match (ChallengeRace pos tid) (RaceI x y) = cmp x pos && cmp y tid 
