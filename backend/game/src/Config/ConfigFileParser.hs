module Config.ConfigFileParser (
    readConfig,
    lookupConfig,
    lookupVar,
    Config(..),
...
lookupConfig sectionName configData = List.lookup sectionName configData
...
arrayParser = between (char '[') (char ']') (valuesParser) >>= return.ArrayC

valuesParser :: Parser [Config]
valuesParser = sepEndBy (valueParser) (many (char ','))

valueCommaParser :: Parser Config
valueCommaParser = valueParser <* (skipMany (char ','))

valueParser :: Parser Config
valueParser =  stringParser <|> floatParser <|> integerParser <|> booleanParser <|> arrayParser
