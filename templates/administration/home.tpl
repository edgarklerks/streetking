<div>
	<ul>
		<li><a href="#part_model/get" class="button" module="PART_MODEL_LIST">part models</a></li>
		<li><a href="#part_type/get" class="button" module="PART_TYPE_LIST">part types</a></li>
		<li><a href="#part_instance/get" class="button" module="PART_INSTANCE_LIST">part instance</a></li>		
		<li><a href="#part_modifier/get" class="button" module="PART_MODIFIER_LIST">part modifier</a></li>
	</ul>
	
	<ul>
		<li><a href="#car_model/get" class="button" module="CAR_MODEL_LIST">car model</a></li>
		<li><a href="#car_instance/get" class="button" module="CAR_INSTANCE_LIST">car instance</a></li>
		<li><a href="#car_options/get" class="button" module="CAR_OPTIONS_LIST">car options</a></li>
	</ul>
	
	<ul>
		<li><a href="#personnel/get" class="button" module="PERSONNEL_LIST">personnel</a></li>
		<li><a href="#personnel_instance/get" class="button" module="PERSONNEL_INSTANCE_LIST">personnel instance</a></li>
	</ul>
	
	<ul>
		<li><a href="#continent/get" class="button" module="CONTINENT_LIST">continent</a></li>
		<li><a href="#city/get" class="button" module="CITY_LIST">city</a></li>
		<li><a href="#track/get" class="button" module="TRACK_LIST">tracks</a></li>
		<div><a href="#track_time/get" class="button" module="TRACK_TIME_LIST">track times</a></div>
	</ul>
	
	<div><a href="#account/get" class="button" module="PROFILE_LIST">Manage profiles</a></div>
	<div><a href="#garage/get" class="button" module="GARAGE_LIST">garages</a></div>
	
	<div><a href="#challenge_type/get" class="button" module="CHALLENGE_TYPE_LIST">challenge types</a></div>
	<div><a href="#transaction/get" class="button" module="TRANSACTION_LIST">transactions</a></div>
	<div><a href="#config/get" class="button" module="CONFIG_LIST">config</a></div>
	<!-- not finished -->
	<!--<div><a href="#challenge_accept/get" class="button" module="CHALLENGE_ACCEPT_LIST">challenge accept</a></div>-->
	<!--<div><a href="#challenge/get" class="button" module="CHALLENGE_LIST">challenges</a></div>-->
	<!-- not exist -->
	<br>
	
</div>